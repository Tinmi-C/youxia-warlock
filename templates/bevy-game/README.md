...
## 代码沉淀准则（先留可装配接缝，不提前造无人消费的通用层）

**自己写的玩法模块怎么沉淀？**

| 时机 | 做法 |
|------|------|
| 第 1 次 | 写进当前游戏正常用；但把**可装配接缝**留标准（SystemSet 阶段 / 插件边界 / 消息与组件契约）——即使暂时只有一个游戏消费 |
| 第 2 次 | 从已留好的接缝处**机械抽出**独立 crate，沉淀到 `engine/`（独立 git 仓库 + 版本号），本游戏通过 `path` 依赖引入 |
