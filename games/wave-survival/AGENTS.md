...
- **进行中**：卡 15 MonsterFacing 已实现（feat `3a15cf2`，2026-08-27，40 个回归全绿；
  数据链数字化断言完成，转身流畅度视觉项待人工跑游戏验收）。`Heading` 观测组件 +
  `derive_heading`（逻辑链尾）+ wrapper yaw 恒定角速度平滑（540°/s 最短弧），
  物理零交互（只转场景子实体）。
...
- **近期目标**：卡 15 待人工视觉验收（跑 `cargo run` 看转身流畅度）→ 推送 → 「UI 正式化」立卡；音效按 GDD 后置
