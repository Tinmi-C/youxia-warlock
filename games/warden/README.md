...
已跑通「单塔单波 · 战斗（双速节奏）」+ **融合** + **波次间「三选一」**：L 形路径 + 基地 + 8 塔位，买塔放置，
敌人沿路径推进，塔自动开火，命中扣血/漏怪扣基地血，金币赚/花，波次推进 + 输赢，10 波；`F` 融合（换定位+腾 1 格，手续费 20%）；
每波结算后 `1/2/3` 三选一。快捷键见上。
5 种塔获取来源（开局手牌/商店保底/掉落）/商店 bevy_ui/Meta/治疗怪为后续卡（见 `docs/capability-cards.md`）。
